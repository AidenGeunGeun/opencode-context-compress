...
description: Build, debug, review, or release the opencode-context-compress TypeScript plugin. Use for changes involving `/compress manage`, `/compress auto`, `/compress squash`, the deterministic `compress` tool, the authorized `squash` tool, automatic compression policy or cooldowns, protectedTurns, prompt transforms, management-turn cleanup, Session Goal continuation markers and overflow recovery, per-session persistence, OpenCode hooks or SDK compatibility, plugin configuration, generated prompts, tests, or committed `dist/` artifacts in this repository.
---
...
- Start at `index.ts` for registered hooks, the `compress` and `squash` tools (both gated on `tools.compress.permission`), config mutation, and feature enablement.
- Use `lib/hooks.ts` for transform, slash-command routing (`manage` / `auto` / `squash`), and chat.message variant caching. Goal continuations do not bound open management turns.
- Use `lib/commands/manage.ts` and `lib/auto-compression.ts` for manual and automatic management-turn initiation. Both open one-call reminders requiring `compress({ summary, topic })`; only the `compress` tool gates those starts. Automatic turns may skip when the protected tail covers all selectable history. Overflow recovery stages one automatic turn with `goalOverflowRecovery`.
- Use `lib/commands/squash.ts` and `lib/tools/squash.ts` for explicit `/compress squash` block-range maintenance. The `squash({ from, to, summary, topic })` tool is authorized only by the current user's active squash turn; it replaces one contiguous positional `[bN]` range of at least two existing blocks and never changes ordinary deterministic selection.
- Use `lib/messages/blocks.ts` for shared canonical anchor-order, positional `[bN]` labels, and model-visible block formatting (labels are never persisted).
...
- Edit prompt sources in `lib/prompts/*.md`; never hand-edit `_codegen` or `dist` as the source of truth. Ordinary agent-facing prompts describe only the current `compress` happy path; range language belongs only in the dedicated squash prompts (`squash.md`, `squash-system.md`).
...
- Keep management/automatic start gated on `compress` alone. Autonomous normal-turn compression remains a prompt-level prohibition (`lib/prompts/compress.md`); runtime only requires a visible user owner of the executing call, not an explicit compression request in message text. `squash` stays fail-closed at runtime to an active `/compress squash` turn.
