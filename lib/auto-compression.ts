...
function messageContainsCompressCall(message: WithParts | undefined): boolean {
...
export function createAutomaticCompressionEventHandler(
    ...
    return async (input: { event?: { type?: string; properties?: { info?: AssistantMessageInfo } } }) => {
        ...
        if (contextTokens < threshold.thresholdTokens) return
        ...
        try {
            ...
            if (findActiveManagementTurn(state, messages)) return
            if (messageContainsCompressCall(messages.find((message) => message.info.id === info.id))) return
            ...
            const started = await startManagementTurn({
                ...
                systemPrompt: renderAutomaticSystemPrompt(flags, {
                    ...
                    threshold_reason: formatThresholdReason(
                        threshold,
                        config.autoCompression.contextWindowRatio,
                    ),
                }),
                source: "automatic",
