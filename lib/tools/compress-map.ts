            ...
            try {
                return await stateManager.runExclusive(sessionId, async () => {
                    ...
                    if (!state.persistenceSynchronized) {
                        throw new Error(
                            "compress_map could not load saved session state. No new map became authoritative; the last successfully returned current-turn map, if any, remains pinned.",
                        )
                    }
                    ...
                    if (!activeTurn) {
                        throw new Error(
                            "compress_map is available only inside a current compression-management turn. Nothing changed. Only the user can authorize manual compression by running `/compress manage`.",
                        )
                    }
                    if (
                        activeTurn.turn.source === "automatic" &&
                        getPostCompressionCooldownRemaining(state, rawMessages) > 0
                    ) {
                        throw new Error(
                            "Automatic compression is still in its post-compression cooldown. No new map became authoritative; only the user can override the cooldown by running `/compress manage`.",
                        )
                    }
                    ...
                    const candidateState: SessionState = {
                        ...state,
                        compressionMapSnapshot: createCompressionMapSnapshot(
                            activeTurn.turn.triggerMessageId,
                            contextMap,
                        ),
                    }
                    ...
                    if (!persisted) {
                        throw new Error(
                            "compress_map could not save this snapshot. No new map became authoritative; the last successfully returned current-turn map, if any, remains pinned.",
                        )
                    }
                ...
                if (error instanceof Error && /compress_map/.test(error.message)) throw error
                const message = error instanceof Error ? error.message : String(error)
                throw new Error(
                    `compress_map could not fetch and pin a map: ${message}. No new map became authoritative; the last successfully returned current-turn map, if any, remains pinned.`,
                )
            }
        },
    })
