Do not call this tool autonomously. Tool availability alone is not authorization. Call it only when the current turn contains either a manual/plugin-initiated compression-management reminder or an explicit user request in the current message to inspect or compress context. Do not infer authorization from context size, task length, or perceived usefulness.
