Use this tool as a fallback during a manual or plugin-initiated compression-management turn to fetch the current compression map. The current `<compress-context-map>` snapshot is normally already provided with the reminder, so this tool is not needed in the ordinary path.
