Use this tool during normal work or a manual/plugin-initiated compression-management turn, after `compress_map` has successfully returned the authoritative snapshot for the current turn. It folds one selected historical span into one durable summary block.
