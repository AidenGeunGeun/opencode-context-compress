Use this tool only during a current manual or plugin-initiated automatic compression-management turn, after `compress_map` has successfully returned the authoritative snapshot for that turn. It folds one selected historical span into one durable summary block.
