...
function deriveProtectedTailMessageIds(messages: WithParts[], protectedTurns: number): string[] {
    if (protectedTurns <= 0 || messages.length === 0) return []
    ...
    for (let index = messages.length - 1; index >= 0; index--) {
        ...
        if (stepCount >= protectedTurns) break
    }
 ...
 */
export function selectDeterministicCompressionSpan(
    rawHistory: WithParts[],
    state: SessionState,
    logger: Logger,
    protectedTurns: number,
): DeterministicCompressionSpan {
    const { transformed, syntheticMap } = transformMessagesForSearch(rawHistory, state, logger)
    const representedBlockAnchors = new Set(
        [...syntheticMap.values()].map((summary) => summary.anchorMessageId),
    )
    const missingBlock = state.compressSummaries.find(
        (summary) => !representedBlockAnchors.has(summary.anchorMessageId),
    )
    if (missingBlock) {
        throw new Error(
            "compress could not reconcile an existing compressed block with the current transcript. Nothing was compressed.",
        )
    }
    ...
    const existingBlockMessageIds = new Set<string>()
    for (const summary of state.compressSummaries) {
        existingBlockMessageIds.add(summary.anchorMessageId)
        for (const messageId of summary.messageIds) existingBlockMessageIds.add(messageId)
    }
    ...
    const candidates = transformed
        ...
        .filter(
            (message) =>
                !syntheticMap.has(message.info.id) &&
