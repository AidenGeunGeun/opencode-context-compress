#!/usr/bin/env bash
#
# Context Compress Token Cache Test Script
# Tests how context compression affects token caching across different providers/models
#
# Usage:
#   ./test-compress-cache.sh [OPTIONS]
#
# Options:
...
#   Terminal 1: ./test-compress-cache.sh --provider anthropic
...
log_section "Context Compress Token Cache Test"
...
for provider in "${PROVIDERS[@]}"; do
    ...
    for codebase_entry in "${CODEBASES[@]}"; do
        ...
        if [[ -z "$SESSION_ID" ]]; then
            ...
            display_cmd="${BASE_CMD} -m '${model}' --title 'Context Compress Test: ${provider}' '<prompt>'"
        else
            ...
            if [[ -z "$SESSION_ID" ]]; then
                ...
                if [[ "$USE_SERVER" == "true" ]]; then
                    opencode run --attach "http://localhost:${SERVER_PORT}" \
                        -m "${model}" \
                        --title "Context Compress Test: ${provider}" \
                        --format json \
                ...
                if [[ -z "$SESSION_ID" ]]; then
                    ...
                    SESSION_ID=$("${SCRIPTS_DIR}/opencode-find-session" "Context Compress Test: ${provider}" 2>/dev/null | head -1 || echo "")
                fi
    ...
    if [[ "$DRY_RUN" == "false" ]] && [[ -n "$SESSION_ID" ]]; then
        ...
        # Compression stats
        "${SCRIPTS_DIR}/opencode-compress-stats" --session "$SESSION_ID" > "${provider_dir}/compress_stats.txt" 2>&1 || true
        "${SCRIPTS_DIR}/opencode-compress-stats" --session "$SESSION_ID" --json > "${provider_dir}/compress_stats.json" 2>&1 || true
        ...
        echo "  $ ${SCRIPTS_DIR}/opencode-compress-stats --session \$SESSION_ID > ${provider_dir}/compress_stats.txt"
    fi
done
