...
function hasToolPart(message, predicate) {
    const parts = Array.isArray(message.parts) ? message.parts : [];
...
function extractEmbeddedUserText(message) {
    const match = EMBEDDED_USER_MESSAGE.exec(getMessageText(message));
    const text = match?.[1]?.trim();
    return text ? text : undefined;
}
function isStatusNotificationMessage(message) {
    if (message.info.role !== "user" || !isIgnoredUserMessage(message)) {
        return false;
    }
    const text = getMessageText(message);
...
function isCompressToolCallMessage(message) {
    return message.info.role === "assistant" && hasToolPart(message, isCompressToolName);
}
function isCompressionOnlyUserRequest(message) {
    if (message.info.role !== "user" || isIgnoredUserMessage(message)) {
        return false;
    }
...
export function buildLegacyResidueSuppressionPlan(rawMessages) {
...
//# sourceMappingURL=legacy-residue.js.map