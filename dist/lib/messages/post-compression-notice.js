 ...
 */
function isResumableAgentTurn(info) {
 ...
 */
export function isPostCompressionNoticeDue(state, rawMessages) {
    ...
    if (isResumableAgentTurn(anchor.info)) {
        ...
        for (let i = tailStart; i < anchorParts.length; i++) {
            if (isAgentWorkPart(anchorParts[i]))
                return false;
        }
    }
    for (let i = anchorIndex + 1; i < rawMessages.length; i++) {
        ...
        if (message.info.role !== "assistant")
            continue;
        if (!isResumableAgentTurn(message.info))
            continue;
        if ((message.parts ?? []).some(isAgentWorkPart))
            return false;
    }
    return true;
}
 ...
 */
export function appendPostCompressionNotice(messages, baseUserMessage) {
    messages.push(createSyntheticUserMessage(baseUserMessage, renderPostCompressionNotice(), baseUserMessage.info.variant));
}
//# sourceMappingURL=post-compression-notice.js.map