        ...
        async execute(args, toolCtx) {
            ...
            const outcome = await stateManager.runExclusive(sessionId, async () => {
                ...
                if (!state.persistenceSynchronized) {
                    throw new Error("compress could not load saved session state - no compression was performed");
                }
            ...
            const updatedContextMap = buildContextMap(outcome.rawMessages, state, logger, outcome.currentParams.providerId);
            const storedBlockId = updatedContextMap.entries.find((entry) => entry.kind === "block" &&
                entry.anchorMessageId === outcome.anchorMessageId)?.key;
            ...
            return buildCompressReceipt(outcome.topic, storedBlockId, outcome.continueTask);
        },
    });
}
